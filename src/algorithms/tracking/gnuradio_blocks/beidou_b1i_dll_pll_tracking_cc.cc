...
{
    return beidou_b1i_dll_pll_tracking_cc_sptr(new BeiDou_B1i_Dll_Pll_Tracking_cc(if_freq,
...
BeiDou_B1i_Dll_Pll_Tracking_cc::BeiDou_B1i_Dll_Pll_Tracking_cc(
    ...
    float pll_bw_hz,
    float dll_bw_hz,
    float early_late_space_chips) : gr::block("BeiDou_B1i_Dll_Pll_Tracking_cc", gr::io_signature::make(1, 1, sizeof(gr_complex)),
...
//    std::cout << "early_late_space_chips: " << early_late_space_chips << std::endl;
    ...
    d_ca_code = static_cast<gr_complex *>(volk_gnsssdr_malloc(static_cast<int>(BEIDOU_B1I_CODE_LENGTH_CHIPS) * sizeof(gr_complex), volk_gnsssdr_get_alignment()));
...
void BeiDou_B1i_Dll_Pll_Tracking_cc::start_tracking()
{
    ...
    multicorrelator_cpu.set_local_code_and_taps(static_cast<int>(BEIDOU_B1I_CODE_LENGTH_CHIPS), d_ca_code, d_local_code_shift_chips);
...
int BeiDou_B1i_Dll_Pll_Tracking_cc::save_matfile()
{
    // READ DUMP FILE
    ...
    if (d_dump)
        {
            ...
            BeiDou_B1i_Dll_Pll_Tracking_cc::save_matfile();
            if (d_channel == 0)
                {
    ...
    if (d_enable_tracking == true)
        {
            ...
            d_code_freq_chips = BEIDOU_B1I_CODE_RATE_HZ + ((d_carrier_doppler_hz * BEIDOU_B1I_CODE_RATE_HZ) / BEIDOU_B1I_FREQ_HZ);
            //carrier phase accumulator for (K) doppler estimation
            d_acc_carrier_phase_rad -= BEIDOU_TWO_PI * d_carrier_doppler_hz * BEIDOU_B1I_CODE_PERIOD;
            //remanent carrier phase to prevent overflow in the code NCO
            d_rem_carr_phase_rad = d_rem_carr_phase_rad + BEIDOU_TWO_PI * ( d_if_freq + d_carrier_doppler_hz ) * BEIDOU_B1I_CODE_PERIOD;
            ...
            code_error_filt_secs = (BEIDOU_B1I_CODE_PERIOD * code_error_filt_chips) / BEIDOU_B1I_CODE_RATE_HZ; //[seconds]
            ...
            if (d_cn0_estimation_counter < FLAGS_cn0_samples)
            ...
            else
                {
                    d_cn0_estimation_counter = 0;
                    // Code lock indicator
                    d_CN0_SNV_dB_Hz = cn0_svn_estimator(d_Prompt_buffer, FLAGS_cn0_samples, d_fs_in, BEIDOU_B1I_CODE_LENGTH_CHIPS);
